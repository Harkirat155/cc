...
const MenuPanel = ({ onReset, onNewGame }) => {
  ...
  return (
    <div
      ref={menuRef}
      className={`fixed left-1/2 bottom-4 transform -translate-x-1/2 bg-white rounded-lg shadow-lg transition-all duration-300 ${expanded ? 'p-4' : 'p-2'} z-30`}
      ...
      {collapsed && !expanded ? (
        ...
        <div className={`flex ${width > 600 ? 'flex-row' : 'flex-col'} gap-2 items-center justify-center w-full`}>
          <div className="flex gap-2 items-center justify-center w-full">
            <button
              className={`py-2 px-6 min-w-[120px] bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-200 border border-gray-300 whitespace-nowrap text-center ${width > 600 ? 'rounded-r-none' : 'mb-2'}`}
            ...
            <button
              className={`py-2 px-6 min-w-[120px] bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-200 border border-gray-300 whitespace-nowrap text-center ${width > 600 ? 'rounded-l-none' : 'mb-2'}`}
